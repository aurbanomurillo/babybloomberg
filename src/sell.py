...
class SellStrategy(Strategy):
    ...
    def __init__(
        ...
        If the provided start date lacks data, the strategy automatically advances 
        the start date to the next available trading day to ensure the initial 
        ...
        Args:
            ...
            threshold (tuple[float, float] | float): The price trigger.
                If float, it is the exact price to trigger a sell.
                If tuple, it represents a price range (min, max) to trigger a sell.
            sizing_type (str, optional): Sizing type. Defaults to "static".
            name (str, optional): Strategy name. Defaults to "undefined_static_sell_strategy".
        """
    ...
    def check_and_do(
        ...
        If the condition is met, a sell order is executed.
    ...
    def execute(self) -> None:
        ...
        Iterates through the simulation dates. If the stock inventory runs out 
        (NotEnoughStockError), the trade is closed immediately. The position is 
        forcefully closed at the end of the simulation period.
        """
    ...
    def execute_and_save(
        ...
        Runs the strategy day-by-day utilizing valid trading days. It specifically 
        handles `NotEnoughStockError` by immediately closing the trade and stopping 
        the simulation. Daily equity (Cash + Stock Value) is calculated and saved 
        to the database.
...
class DynamicSellStrategy(SellStrategy):
    ...
    determined by `trigger_lookback`. This is useful for dynamic exits like 
    Trailing Stops (selling on dips) or dynamic Take Profits (selling on spikes).
    ...
    def __init__(
        ...
        Args:
            ...
            sf (StockFrame): Data manager.
            amount_per_trade (float): Value to sell per signal.
            threshold (tuple[float, float] | float): Percentage variation to trigger sell.
                - Ex: -0.05 implies selling if price drops 5% (Stop Loss/Trailing).
                - Ex: 0.10 implies selling if price rises 10% (Take Profit).
            trigger_lookback (str, optional): Lookback period for reference price. 
                Defaults to "1 day".
            sizing_type (str, optional): Sizing type. Defaults to "static".
            name (str, optional): Strategy name. Defaults to "undefined_dynamic_sell_strategy".

        Raises:
            NotValidIntervalError: If threshold implies a drop of 100% or more (<= -1.0).
        """
    ...
    def check_and_do(
        ...
        If threshold is positive, sells if price rose >= X% (Take Profit).
        If threshold is negative, sells if price fell >= X% (Stop Loss).
